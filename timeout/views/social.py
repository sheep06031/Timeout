...
def flag_post(request, post_id):
    ...
    if reason not in [c[0] for c in PostFlag.Reason.choices]: reason = 'other'
    _, created = PostFlag.objects.get_or_create(post=post, reporter=request.user, defaults={'reason': reason, 'description': description})
    if created: messages.success(request, 'Post has been flagged for review.')
    else: messages.info(request, 'You have already flagged this post.')
    return redirect('social_feed')
...
def ban_user(request, username):
    ...
    if not request.user.is_staff: return HttpResponseForbidden('Staff access required.')
    target = get_object_or_404(User, username=username)
    if target.is_staff:
